            ...
            if (replicas == 1) { // standalone
                ...
                for (int i = 0; i < replicas; i++) {
                    ...
                    try {
                        ...
                        String cluster = pod.getMetadata().getLabels().get(Labels.STRIMZI_CLUSTER_LABEL);
                        Secret brokersSecret = secretOperations.get(namespace, KafkaCluster.brokersSecretName(cluster));
                        ...
                        CertAndKey kafkaCertKey = Ca.asCertAndKey(brokersSecret, cluster + "-kafka-0.key", cluster + "-kafka-0.crt");
                        X509Certificate caCertCO = (X509Certificate) x509.generateCertificate(new ByteArrayInputStream(decoder.decode(clusterCaCertificateSecret.getData().get("ca.crt"))));
                        ...
                        kmf.init(setupKeyStore(clusterCaKeySecret, x509, password, caCertCO, kafkaCertKey), password);
                        ...
                        leader = 0;
                        log.debug("Error while getting Zookeeper leader");
                    }
                }
            }
            f.complete(leader);
